...
async function confirmBan(reason) {
    ...
    try {
        ...
        const res = await fetch(`${config.public.apiBase}admin/users/${user.id}/blacklist`, {
            method: 'POST',
            headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
...
async function confirmUnban() {
    ...
    try {
        ...
        const res = await fetch(`${config.public.apiBase}admin/users/${user.id}/unblacklist`, {
            method: 'POST',
            headers: {
                Accept: 'application/json',
                ...(token ? { Authorization: `Bearer ${token}` } : {}),
        ...
        if (!res.ok) {
            throw new Error('Failed to unblacklist user')
        }
