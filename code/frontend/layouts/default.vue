<template>
    ...
    >
      <div
        v-if="openNotif"
        ref="notifPanel"
        class="absolute top-full right-0 mt-3 w-[360px] max-w-[90vw] max-h-[70vh]
        bg-white border border-gray-200 rounded-2xl shadow-xl overflow-hidden z-[60]"
        @click.stop
      >
        <!-- Header -->
        <div class="flex items-center justify-between px-4 py-3 ">
          <h3 class="text-lg font-semibold">Notification</h3>
          <button @click="openNotif = false">✕</button>
        </div>
              ...
              <div class="flex items-start gap-3">
                ...
                <div class="relative shrink-0">
                  ...
                  <div
                    ...
                    class="absolute right-0 top-6 w-44 bg-white border rounded shadow z-50"
                    @click.stop
                  >
                    <button
                      class="block w-full px-3 py-2 text-sm hover:bg-gray-50"
                      @click="markAsRead(n)"
                    >
                      ทำเครื่องหมายอ่านแล้ว
                    </button>
                    <button
                      class="block w-full px-3 py-2 text-sm text-red-600 hover:bg-red-50"
